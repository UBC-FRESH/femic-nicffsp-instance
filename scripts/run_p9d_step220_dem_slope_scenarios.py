...
STEP210_SOURCE_AREA_HA = 129_210.10262294934
EXISTING_STEP290_CURRENT_THLB_HA = 124_568.76502080733
MP11_STEP290_CURRENT_THLB_HA = 120_099.0
...
def main() -> None:
    ...
    payload = {
        ...
        "existing_step290_current_thlb_ha": EXISTING_STEP290_CURRENT_THLB_HA,
        "mp11_step290_current_thlb_ha": MP11_STEP290_CURRENT_THLB_HA,
...
def _scenario_row(
    *,
    ...
    step290_proxy_area = EXISTING_STEP290_CURRENT_THLB_HA - deduction_ha
    return {
        ...
        "step290_direct_proxy_area_ha": step290_proxy_area,
        "step290_direct_proxy_delta_to_mp11_ha": (
            step290_proxy_area - MP11_STEP290_CURRENT_THLB_HA
        ),
    }
...
def _write_markdown(payload: dict[str, object], path: Path) -> None:
    ...
    lines = [
        ...
        (
            "- Existing P9RF Step 290 Current THLB before Step 220 repair: "
            f"`{payload['existing_step290_current_thlb_ha']:.3f} ha`"
        ),
        "",
        ...
        (
            "- Direct Step 290 proxy delta if applied without downstream overlap "
            f"rebuild: `{whole['step290_direct_proxy_delta_to_mp11_ha']:.3f} ha`"
        ),
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "The direct Step 290 proxy is only a triage number. Final Current "
                "THLB deltas must come from rerunning the P9RF resultant-fragment "
                "netdown with the selected Step 220 rule physically applied."
            ),
            "",
        ]
    )
